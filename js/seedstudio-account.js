var uid=localStorage.getItem('ss_id')||'',cr=0,ti='free',app=null;

function call(a,d){return app.callFunction({name:'api',data:{action:a,data:d||{}}}).then(function(r){var v=r.result;if(v.error)throw new Error(v.error);return v;});}

function init(cb){
  if(typeof tcb==='undefined'){console.error('tcb not loaded');cb(null,null);return;}
  app=tcb.init({env:'seedstudio-d6gcgy3nwa8b4e1ca'});
